...
function M.encode_frame(seq, map, frame)
    return "F|"..seq.."|"..require("mp.codec").encode({map,frame})
end
...
function M.decode(line)
    ...
    local seq, payload = line:match("^F|(%d+)|(.*)$")
    if seq then
        seq=tonumber(seq)
        if not seq or seq>9007199254740991 then return nil end
        local decoded = require("mp.codec").decode(payload)
        if type(decoded)=="table" and #decoded==2 and type(decoded[1])=="string" and type(decoded[2])=="table" then
            return {type="frame",seq=seq,mapId=decoded[1],frame=decoded[2]}
        end
        return nil
    end
