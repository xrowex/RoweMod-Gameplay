...
$runDir = Join-Path $env:TEMP ("RoweModMP\run-" + [guid]::NewGuid().ToString("N"))
$mailA = Join-Path $runDir "A"
$mailB = Join-Path $runDir "B"
...
$hostProc = Start-Process -FilePath $py.Source -ArgumentList @(
    "`"$bridge`"", "host", "--port", "$port", "--mailbox", "`"$mailA`"", "--name", "PlayerA"
) -PassThru -WindowStyle Hidden -RedirectStandardOutput (Join-Path $runDir "host.log") -RedirectStandardError (Join-Path $runDir "host-errors.log")

Start-Sleep -Milliseconds 400
if ($hostProc.HasExited) { throw "Host bridge failed. Check $runDir\host-errors.log; close any old test bridges first." }

$joinProc = Start-Process -FilePath $py.Source -ArgumentList @(
    "`"$bridge`"", "join", "--host", "127.0.0.1", "--port", "$port", "--mailbox", "`"$mailB`"", "--name", "PlayerB"
) -PassThru -WindowStyle Hidden -RedirectStandardOutput (Join-Path $runDir "join.log") -RedirectStandardError (Join-Path $runDir "join-errors.log")

Start-Sleep -Milliseconds 600

function Start-GameInstance([string]$Mailbox, [string]$Name, [string]$Role, [int]$X, [int]$Y) {
    $args = @(
        '-ExecCmds="Slate.RequireFocusForGamepadInput 1"',
        "-windowed",
        "-ResX=1280",
        "-ResY=720",
        "-WinX=$X",
    ...
    Start-Process -FilePath "cmd.exe" -ArgumentList @("/c", $cmd) -WorkingDirectory $win64 -WindowStyle Hidden
}
...
Write-Host "  2) Press F10, type rowemod mp on, then Enter"
...
Write-Host "Bridge logs: $runDir"
@{ hostPid = $hostProc.Id; joinPid = $joinProc.Id; runDir = $runDir } |
    ConvertTo-Json | Set-Content (Join-Path $runDir "processes.json")
Write-Host "After closing both games, stop bridges: tools\mp_stop_local.cmd"
