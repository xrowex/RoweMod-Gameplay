...
class ProtocolTests(unittest.TestCase):
    def test_heartbeat_tracks_game_map_and_ignores_requested_map(self):
        with tempfile.TemporaryDirectory() as tmp:
            hub = rmp.UdpHub(0, rmp.Mailbox(Path(tmp)), "PlayerA", None, map_id="DualLocal")
            try:
                hub._observe_game_line("H|1|PlayerA|2|OutdoorSkatepark")
                self.assertEqual(hub._hello_line(), "H|0|PlayerA|3|OutdoorSkatepark")
                hub._observe_game_line("MREQ|2|OtherPark")
                self.assertTrue(hub._hello_line().endswith("|OutdoorSkatepark"))
                hub._observe_game_line("M|3|OtherPark")
                self.assertTrue(hub._hello_line().endswith("|OtherPark"))
                hub._observe_game_line("Bye|4")
                self.assertTrue(hub._hello_line().endswith("|unknown"))
            finally:
                hub.sock.close()
...
class MailboxTests(unittest.TestCase):
    def test_restarted_game_can_reuse_consumed_filename(self):
        with tempfile.TemporaryDirectory() as tmp:
            box = rmp.Mailbox(Path(tmp))
            path = box.out_dir / "000001.msg"
            path.write_text("H|1|A|2|Park\n", encoding="utf-8")
            self.assertEqual(box.poll_out(), ["H|1|A|2|Park"])
            path.write_text("Bye|2\n", encoding="utf-8")
            self.assertEqual(box.poll_out(), ["Bye|2"])
            self.assertEqual(box.poll_out(), [])
